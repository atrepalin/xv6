...
kmalloc(int size)
{
  ...
  if(size <= MAX_KM_SIZE) {
    ...
    int npages = (size + sizeof(struct kmpage) + PGSIZE - 1) / PGSIZE;
    char *mem = kalloc_npages(npages);
    if(mem == 0)
      return 0;

    struct kmpage *hdr = (struct kmpage*)mem;
    hdr->blocksize = -1;
    hdr->nblocks = npages;

    return mem + sizeof(struct kmpage);
  }
}
...
kmfree(void *ptr)
{
  ...
  if(hdr->blocksize > 0) {
    ...
    release(&kmem[idx].lock);
  } else if(hdr->blocksize == -1) {
    kfree_npages(hdr, hdr->nblocks);
  }
}